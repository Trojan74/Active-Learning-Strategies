...
class CIFAR10ClassifierModule(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.dim2 = 48
        self.dim3 = 64
        self.dim4 = 128  # Dimension for the additional convolutional layer
        ...
        self.conv_layer4 = nn.Conv2d(in_channels=self.dim3, out_channels=self.dim4, kernel_size=3, stride=1, padding=1)  # Additional layer
        self.batchnorm4 = nn.BatchNorm2d(self.dim4)
        self.relu4 = nn.ReLU()
        ...
        self.fc_layer1 = nn.Linear(self.dim4*4, self.dim4)      # 512 -> 64
        self.relu5 = nn.ReLU()
        self.dropout1 = nn.Dropout(p=0.3)

        self.fc_layer2 = nn.Linear(self.dim4, self.dim4 // 2)  # 64 -> 32
        self.relu6 = nn.ReLU()
        self.dropout2 = nn.Dropout(p=0.3)

        self.fc_layer3 = nn.Linear(self.dim4 // 2, num_classes)   # 32 -> 10
    ...
    def forward(self, X, **kwargs):
        ...
        # Convolutional Layer 4 (Additional)
        X = self.conv_layer4(X)
        X = self.batchnorm4(X)
        X = self.relu4(X)
...
initial_train_size = int(len(X_train)*0.10) # Small initial training set
    ...
    def active_learning_training(self):
        ...
        for cycle in range(1, self.num_cycles + 1):
            ...
            for epoch in range(self.epochs_per_cycle):
                ...
                metrics["epoch_val_accuracy"].append(val_accuracy)
...
init_epochs = 40
epochs_per_cycle = 20
num_samples_per_cycle = int(initial_train_size*0.1)
batch_size = 64
...
metrics_random = trainer_random.active_learning_training()
...
metrics_plugin = trainer_plugin.active_learning_training()
...
metrics_plugin2 = trainer_plugin.active_learning_training()
...
trainer_laplace = ActiveLearningTrainer(
    model=model,
    criterion=criterion,
    optimizer=optimizer,
    train_data=train_data,
...
metrics_laplace = trainer_laplace.active_learning_training()
...
def plot_metrics(metrics_list, labels):
    ...
    plt.show()
plot_metrics([metrics_plugin,metrics_plugin2,metrics_random], ["Plugin-Approximation - Entropy","Plugin-Approximation - Entropy & Redundancy","Random Sampling"])
#plot_metrics([metrics_plugin,metrics_laplace,metrics_random], ["Plugin-Approximation","Laplace Approximation","Random Sampling"])